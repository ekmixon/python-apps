...
class Secureworks(AppBase):
    ...
    def get_ticket(self, username, password, ticketId, includeWorklogs=False):
        url = f"https://api.secureworks.com/api/ticket/v3/tickets/{ticketId}"
        headers = {
            "Authorization": f"APIKEY {username}:{password}",
            "Content-Type": "application/json",
        }
    ...
    def close_ticket(self, username, password, ticketId, closeCode, worklogContent):
        url = f"https://api.secureworks.com/api/ticket/v3/tickets/{ticketId}/close"
        headers = {
            "Authorization": f"APIKEY {username}:{password}",
            "Content-Type": "application/json",
        }
    ...
    def add_worklog(self, username, password, ticketId, body):
        url = f"https://api.secureworks.com/api/ticket/v3/tickets/{ticketId}/worklogs"
        headers = {
            "Authorization": f"APIKEY {username}:{password}",
            "Content-Type": "application/json",
        }
    ...
    def assign_ticket(self, username, password, ticketId, body):
        url = f"https://api.secureworks.com/api/ticket/v3/tickets/{ticketId}/assign"
        headers = {
            "Authorization": f"APIKEY {username}:{password}",
            "Content-Type": "application/json",
        }
